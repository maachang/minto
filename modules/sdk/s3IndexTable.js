...
// - 文字列は無効なS3キー文字対策・"!"区切り文字との衝突対策として
//   base64でエンコードする(llrtは "base64url" エンコーディング名を
//   サポートしていないため、標準base64後に手動で "+"→"-", "/"→"_",
//   末尾"="除去、というURL-safe変換を行う).
//   ※ 重要な制約: base64エンコードは辞書順を保持しないため、
//     string型カラムをインデックスの先頭カラム(範囲検索対象)にした
//     場合、gt/ge/lt/le(範囲演算子)は使用できない(eq/inのみ許可)。
//     文字列の範囲検索が必要な場合は別途エンコード方式の検討が必要.
///////////////////////////////////////////////
(function () {
    'use strict';
    ...
    // NULL値の予約トークン.
    // "~" はbase64(および手動URL-safe変換後の A-Za-z0-9-_ )に
    // 出現しない文字なので、通常のエンコード結果と衝突しない.
    ...
    // 標準base64をS3キーセーフなURL-safe文字列に変換する.
    // (llrtが "base64url" エンコーディング名を未サポートなため
    //  手動変換する).
    const _toBase64Url = function (buf) {
    ...
    const encodeString = function (value) {
        ...
        return _toBase64Url(Buffer.from(String(value), "utf-8"));
    };
    ...
    // string型はbase64エンコードのため辞書順を保持せず、範囲検索不可.
    const isRangeSupportedType = function (type) {
        return type === "int" || type === "float" || type === "date";
    };
