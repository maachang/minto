...
└── public/
    ...
    ├── mypage.mt.html        (ログイン後ページ、filter.mt.jsで保護される)
    ├── logout.mt.js
    └── filter.mt.js          (認証フィルター。未ログインならgasAuth.redirectToOAuth()
                                で自動的にGASログインへリダイレクトする)
```
...
1. 未ログインで保護対象ページ(`/mypage`)へアクセスすると、`filter.mt.js`が
   セッション無しと判定し、`gasAuth.redirectToOAuth(req, res, "/resultOAuth")`を
   **1回呼ぶだけ**でGASへの署名付きURL(`callbackURL=.../resultOAuth`と、
   現在アクセスしようとしていたパスを自動セットした`srcURL`を含む)を生成し、
   ブラウザをそのURLへリダイレクトする
...
4. `/resultOAuth`(サーバー)が`gasAuth.getOAuthMail(req)`でシグニチャーを検証し、
   ...
   (=手順1でアクセスしようとしていた`/mypage`。無ければ`/mypage`)へリダイレクトする
...
`conf/session.json`のS3バケット名は自環境のものに合わせる。
...
- `modules/auth/gasAuth.js`
  - `redirectToOAuth(request, response, callbackPath)`: **フィルター(認証ガード)
    向けの単発ヘルパー。** 未ログイン判定した箇所からこれを1回呼ぶだけで、
    現在アクセスしようとしていたパス(`request.path()`)を自動的に`srcURL`として
    使い、GASへのoAuth用URLを生成してレスポンスをリダイレクトさせる
    (URL生成とリダイレクトをまとめて行う)。このサンプルの`filter.mt.js`が
    使用している。
  - `executeOAuthURL(request, callbackPath)`: GASへ問い合わせるための署名付きURLを
    「生成するだけ」(リダイレクトは呼び出し元が行う)。`request.params().srcURL`が
    あればoAuth成功後の着地先として使う。ログインページに明示的な「ログイン」
    リンクを置きたい場合など、`redirectToOAuth`より細かく制御したい場合に使う。
