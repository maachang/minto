...
minto では「動的コンテンツ」が利用可能で、これらは１〜５が対象となります。
...
このファイルは、存在する場合、AWS Lambda が **SQSトリガー** によって呼び出された際に実行される内容です。

ブラウザからの通常のリクエスト(URL Function)では実行されず、Lambda関数に SQSトリガー(イベントソースマッピング)を設定した場合の、SQSメッセージ受信時にのみ呼び出されます。

**この機能を用意した目的:**

`modules/s3table` 系のテーブル更新は、URL Function経由で直接呼び出すと複数リクエストから同時多発的に(多方面から)更新が行われる形になります。これに対して、更新処理を一旦SQSに投げて `runSqs.mt.js` 経由で受け取って実行する構成にすることで、更新処理をSQS側で順序立てて(直列に)実行させることができ、多方面からの同時更新による競合を避けられます。

また「runSqs」も１つのみ定義が可能となっています。
- /public/runSqs.mt.js
...
exports.handler = async function (params) {
    // paramsには、SQSメッセージ1件分のbody(JSON.parse済み)が渡されます.
    console.log(params);
}
~~~

Lambdaに設定されたSQSトリガーのイベントは `event.Records` に複数件まとめて渡されますが、minto側で1件ずつ取り出し、各`body`をJSONパースした上で `params` として `runSqs.mt.js` の `handler` に渡し、レコード単位で実行します。

1レコードの処理で例外が発生した場合もそのレコードの失敗としてログ出力(`console.error`)されるのみで、後続レコードの処理は継続されます。

また `filter.mt.js` と同様、`runSqs.mt.js` へのURLパス直接アクセスは許可されず、`status: 403` となります。
...
これによって
- ${MINTO_HOME}/lambda/src/lib/s3sdk.js

ここのモジュールをロードされます。

またmintoコマンドでローカル検証環境での利用の場合は、
- ${MINTO_HOME}/lambda/src/lib
- require("path").resolve() + "/lib" + "/s3sdk.js"
...
### 6. S3をデータストアとして使う(s3table)

mintoでは、S3をデータストアとして使うための中核モジュール群 `modules/s3table`(`s3sdk.js`・`s3MasterTable.js`・`s3IndexTable.js`・`s3Lock.js`・`seqId.js`)を提供しています。
...
テーブルの作成・削除・カラム変更・インデックス変更は`tableTool`コマンド(`conf/table/{master|index}.json`にあるべき定義を記載して実行)で行います。詳しくは以下を参照してください。
...
- [tableToolコマンド](https://github.com/maachang/minto/blob/main/bin/README.md#tabletool-%E3%82%B3%E3%83%9E%E3%83%B3%E3%83%89)
...
- jhtml組み込みタグ説明.
  - <% ... %><br>
    基本的な組み込みタグ情報（制御構文等、出力なし）
  - <%= ... %><br>
    実行結果をHTMLエスケープして出力する組み込みタグ（XSS対策）.
  - <%- ... %><br>
    実行結果をエスケープせずにRaw出力する組み込みタグ.
  - <%# ... %><br>
    コメント用の組み込みタグ.
  - ${ ... }<br>
    実行結果をHTMLエスケープして出力する組み込みタグ.<br>
    <%= ... %> これと内容は同じ.<br>
    ただ利用推奨としては、変数出力時に利用する.
  - !{ ... } / ${! ... }<br>
    実行結果をエスケープせずにRaw出力する組み込みタグ.<br>
    <%- ... %> これと内容は同じ.

- jhtml組み込み機能.
  - `$include`: 別テンプレート（ヘッダー・フッター等の部品）をインクルード（例: `${$include("./parts/header", { title: "ページ名" })}`）<br>
    ※ 詳細は [tools/jhtml.md](../tools/jhtml.md) を参照。
  - `$params`: `$include` で渡されたパラメータオブジェクト
  - `$escape` / `$escapeHtml`: HTML特殊文字（`&`, `<`, `>`, `"`, `'`）をエスケープする関数
  - `$out`: HTML直接出力関数
  - `$request()`: リクエスト情報
  - `$response()`: レスポンス情報
...
<body>
    <%
        const res = await s3sdk.list("testBucket");
