...
- `-c` / `--command`: 実行するコマンド
  ...
  - `describeBackup`: 指定したバックアップ世代の中身(スキーマ・行数)を、
    復元せずに確認する(`master`/`index`両対応、`-n`・`-b`必須)
...
- `describeBackup`は指定したバックアップ世代のスキーマ・行数
  (`target=index`の場合はインデックスエントリ数も)を、復元せずに確認する
- `restoreBackupAs`はバックアップの内容を元とは別のテーブル名(`-d`で指定)
  として新規復元する(クローン用途)。複製先テーブル名が既に存在する場合は
  事故防止のためエラーになる(上書きしたい場合は先に明示的に`dropTable`を
  実行する)
...
#### modules に対応する public(共通ページ・QRコード等)の同時デプロイ

`$MINTO_HOME/public/`以下(minto本体同梱の共通静的コンテンツ・共通ページ。
`tools/webapps.js`のローカル実行時404フォールバック先。例:
`public/auth/mfa/`)も、上記`modules`のpackと同じタイミングで以下のルールで
デプロイzipに含まれます.

- `public/js/`・`public/css/`等、`modules/***`に同名ディレクトリが**存在しない**
  ものは、`--target`の指定に関わらず常にpack化されます.
- `public/auth/`のように`modules/***`(この例では`modules/auth`)に**同名の
  ディレクトリが存在する**ものは、`mtpk -t auth`(または`-t all`)を指定した
  場合のみpack化されます(`modules`側のpack対象選択と対になります)。
  `modules`側と異なり、`public`以下のパス構造(`public/auth/mfa/...`)は
  そのまま維持してpack化され、`.mt.html`はプロジェクト自身の`public`と
  同様にjhtml変換(`.jhtml.js`)されます.
- `modules/***`に対応するディレクトリが無い`public/***`は、`--target`に
  何を指定しても(`all`であっても)pack化されません(`modules`と`public`は
  同名ディレクトリでセットになっているものだけが対象です).
...
プロジェクトの `public/**/*.mt.js`・`public/**/*.mt.html`・`lib/**/*.js` を走査して
`$loadLib("xxx.js")` の呼び出しを検出し、その参照先が以下のいずれかを判定して
レポートするコマンドです。

- プロジェクトの `lib/` 配下に既に存在する(OK。`mtpk`でもそのまま同梱される)
- `lib/` には無いが `$MINTO_HOME/modules/{カテゴリ名}/` 配下に存在する
  (ローカルの `minto` コマンド実行時は `tools/webapps.js` の `modules/` フォール
  バックで動作するが、`mtpk` でのデプロイパッケージには対象カテゴリを
  `-t {カテゴリ名}` で明示指定しない限り含まれない)
- `lib/` にも `modules/` のどのカテゴリにも見つからない(typoまたは独自libファイル
  の配置漏れの可能性)
...
`-v` / `--verbose` を付けると、各参照名がどのファイルから呼び出されているかも
表示します。
...
`lib/` に無く `modules/` 側で見つかったカテゴリがある場合、実行結果の最後に
`mtpk -t xxx -t yyy` のような推奨コマンドが表示されるので、`mtpk`実行前にこの
内容を確認することで、「ローカルでは動くのにLambda上で`$loadLib`が失敗する」
(`-t`指定漏れによる)事故を未然に防げます。

なお、これも正規表現による簡易的な静的解析のため、コメント文中に
`$loadLib("xxx.js")` のような記述があった場合も参照ありとして検出してしまう
点にはご注意ください(実害としては、既に`lib/`にある参照が余分にOK表示される
程度で、必要な`-t`指定の見落としには繋がりません)。
