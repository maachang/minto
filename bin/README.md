...
対して、テーブルの作成・削除・カラム変更・インデックス変更、および`s3IndexTable.js`
専用のバックアップ/リストアを行うコマンドです。
実装は`lambda/src/index.js`側に集約されており、AWSコンソールの「テスト実行」で
以下と同じ形のevent(JSON)を渡して実行することもできます(ローカル実行・Lambda実行
どちらでも全く同じ処理が行われます)。

~~~sh
> tableTool -t <master|index> -c <createTable|dropTable|alterTable|alterIndex|backupTable|restoreTable|listBackups> [-n <テーブル名>] [-b <backupId>]
~~~
...
- `-c` / `--command`: 実行するコマンド
  ...
  - `restoreTable`: 指定した世代のバックアップ内容でテーブルを全置換する
    (`target=index`のみ対応、`-n`・`-b`必須)
  - `listBackups`: 指定した1テーブルの既存バックアップ世代(backupId)一覧を返す
    (`target=index`のみ対応、`-n`必須)
- `-n` / `--table`: `alterIndex`/`backupTable`/`restoreTable`/`listBackups`実行時に
  対象とするテーブル名(必須)
- `-b` / `--backupId`: `restoreTable`実行時に対象とするバックアップ世代ID(必須、
  `backupTable`の実行結果で返る`backupId`を指定する)
...
- `backupTable`は行データ・インデックスエントリ・スキーマ定義を
  `backup/{テーブル名}/{backupId}/`配下(`backupId`は実行時のUnixTimeミリ秒)に
  そのまま複製する(物理コピー方式)。複数世代を保持でき、古い世代の削除は
  現状自動では行われない(必要であれば別途手動でS3オブジェクトを削除する)
- `restoreTable`は指定した世代の内容で現在のテーブル(行データ・インデックス・
  スキーマ)を**全置換**する(差分マージはしない。実行前に現在の内容は
  破棄される)。整合性の取れたバックアップ/リストアを行うには、通常の
  CRUD処理が行われていないメンテナンス時間帯に実行する運用が前提となる
