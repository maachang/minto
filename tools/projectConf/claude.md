...
- **SMS（ショートメッセージ）認証・通知の不採用**:
  - SMS送信サービス（AWS SNS SMSやTwilio等）は従量課金単価が非常に高く（国内宛1通約8円〜15円、国外宛さらに高額）、月額固定費やスパム送信・SMSポンプ攻撃による高額請求リスクを招くため、mintoの基本方針（「固定費ほぼ0円・安全・低リスク」）に反する。また送達元番号（Sender ID）の申請や携帯キャリアの審査・規制対応など導入・運用ハードルが極めて高いため、認証・通知用途にはSMSは基本利用しない（代わりにメール/SESやTOTPアプリ、Google認証/GAS SSO等を利用する）。
