...
- **暗号・セキュリティ乱数の利用ルール**: `createRandom()` や `rand` は Xor128 による高速な疑似乱数生成器（PRNG）であり、暗号論的に安全ではありません。セッションID、トークン、パスワード、ソルト、暗号鍵などのセキュリティ・暗号化用途のランダム値として `createRandom` / `rand` を利用・想定することは厳禁。暗号・セキュリティ用途には必ず `crypto.randomBytes()` や `crypto.getRandomValues()` などの暗号論的疑似乱数（CSPRNG）を使用すること。
