...
test("s3IndexTable: describeTableはテーブル定義を取得する", async () => {
    const db = createDb();
    const table = nextTableName();
    await db.createTable(table, {
        columns: { name: { type: "string" }, age: { type: "int" } },
    ...
    assert.deepEqual(schema.columns, { name: { type: "string" }, age: { type: "int" } });
    assert.deepEqual(schema.indexes, { byName: ["name"] });

    await assert.rejects(() => db.describeTable("no-such-table"));
});

test("s3IndexTable: countはselectを使わず行データのGETなしで件数を数える", async () => {
    const db = createDb();
    const table = nextTableName();
    await db.createTable(table, {
        columns: { status: { type: "string" } },
    ...
    const openCount = await db.count(table, { byStatus: { status: "open" } });
    assert.equal(openCount, 2);

    const allCount = await db.count(table, { byStatus: {} });
    assert.equal(allCount, 3);

    // 行ファイルを直接削除してもcountはインデックスのみを見るため件数に影響しない
    // (selectと違い自己修復もされない。あくまでインデックス上の件数を返す軽量API).
    const listRes = await s3sdk.list(BUCKET, "table/" + table, { noError: false });
    await s3sdk.delete(BUCKET, null, listRes.Contents[0].Key, { noError: false });
    const afterDeleteCount = await db.count(table, { byStatus: {} });
    assert.equal(afterDeleteCount, 3);
});

test("s3IndexTable: transactionはテーブル単位ロックを取得してfnを実行し、成功時はロックを解放する", async () => {
    const db = createDb();
    const table = nextTableName();
    await db.createTable(table, {
        columns: { name: { type: "string" } },
    ...
    // 同じロックキー("index."+テーブル名)を取得できる.
    const s3Lock = require("../../modules/s3table/s3Lock.js");
    const lock = s3Lock.create({ bucket: BUCKET });
    const acquired = await lock.acquire("index." + table);
    assert.equal(acquired, true);
    await lock.release("index." + table);
});

test("s3IndexTable: transactionは実行中に例外が発生してもロックを解放し、既に書き込んだ変更はロールバックしない", async () => {
    const db = createDb();
    const table = nextTableName();
    await db.createTable(table, {
        columns: { name: { type: "string" } },
    ...
    // ロールバックは提供しないため、例外前に書き込んだaliceは残ったまま.
    const rows = await db.select(table, { where: { byName: { name: "alice" } } });
    assert.equal(rows.length, 1);
...
test("s3IndexTable: 既に同じテーブルでロックが取得されている場合、transactionは即座に失敗する", async () => {
    const db = createDb();
    const table = nextTableName();
    await db.createTable(table, { columns: { name: { type: "string" } } });
    ...
    const s3Lock = require("../../modules/s3table/s3Lock.js");
    const lock = s3Lock.create({ bucket: BUCKET });
    await lock.acquire("index." + table);
